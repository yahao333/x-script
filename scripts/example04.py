...
def scale_image_uniform_v2(image_input, scale_factor, rect=None):
    """Scale an image or image region uniformly by the given factor.
    
    Args:
        image_input: Path to image file or PIL Image object
        scale_factor: Float scaling factor (e.g. 1.5 for 150% size)
        rect: Optional tuple of (x, y, width, height) defining region to scale
        
    Returns:
        PIL.Image object containing the scaled image, or None on failure
    """
    try:
        # Handle input image
        if isinstance(image_input, str):
            if not os.path.exists(image_input):
                logger.error(f"Input file does not exist: {image_input}")
                return None
            img = Image.open(image_input)
        else:
            img = image_input
            
        # Crop if rect specified
        if rect:
            x, y, width, height = rect
            if x < 0 or y < 0 or x + width > img.width or y + height > img.height:
                logger.error("Invalid crop rectangle coordinates")
                return None
            img = img.crop((x, y, x + width, y + height))
            logger.debug(f"Cropped image to region: {rect}")
            
        # Calculate new dimensions
        new_width = int(img.width * scale_factor)
        new_height = int(img.height * scale_factor)
        
        logger.debug(f"Scaling image from {img.width}x{img.height} to {new_width}x{new_height}")
...
async def perform_ocr(image_input, rect=None):
    """
    Perform OCR on an image or image region
    
    Args:
        image_input: Path to image file or PIL Image object
        rect: Optional tuple of (x, y, width, height) defining region to process
        
    Returns:
        Recognized text string or None if failed
    """    
    try:
        logger.debug(f"Starting OCR process for {'image object' if isinstance(image_input, Image.Image) else image_input}")
        ...
        scaled_img = scale_image_uniform_v2(image_input, scale_factor, rect)
